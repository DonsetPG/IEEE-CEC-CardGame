...
* réfléchir à des stratégies en fonction de si on est JoueurBOT2 1 ou JoueurBOT2 2 : draft/summon/attack. 
*
*
*
*
...
class CardBOT2{
	...
	private int idCardBOT2;
	private int id;
	private int location;
	private int CardBOT2Type;
	private int cost;
	private int attack;
	private int defense;
	...
	public CardBOT2(int idCardBOT2,int id,int location,int CardBOT2Type,int cost,int attack,int defense,String abilities,int hpChange,int hpChangeOpponent,int CardBOT2Draw,int placeDraft){
		this.idCardBOT2 = idCardBOT2;
		this.id = id;
		this.location = location;
		this.CardBOT2Type = CardBOT2Type;
		this.cost = cost;
		this.attack = attack;
		...
		this.CardBOT2Draw = CardBOT2Draw;
	...
	public CardBOT2(int hp){
		this.idCardBOT2 = -1;
		this.id = -1;
		this.location = 100;
		this.CardBOT2Type = 100;
		this.cost = 1;
		this.attack = 0;
		this.defense = hp;
	...
	public CardBOT2(CardBOT2 c){
		this.idCardBOT2 = c.idCardBOT2;
		this.id = c.id;
		this.location = c.location;
		this.CardBOT2Type = c.CardBOT2Type;
		this.cost = c.cost;
		...
		this.CardBOT2Draw = c.CardBOT2Draw;
	...
	public CardBOT2(CardBOT2 c,int a){
		this.idCardBOT2 = c.idCardBOT2;
		this.id = c.id;
		this.location = c.location;
		this.CardBOT2Type = c.CardBOT2Type;
	...
	public int getCardBOT2Draw(){
		return this.CardBOT2Draw;
	}
	...
	public double scoreTest(int n,int mineHp,int opponentHp){
		...
		if (this.CardBOT2Draw > 0 && mineHp < 15 && n == 0){
			score += 4;
		}
		if (this.breaktrough){
	...
	public double scoreBis(int hp,int hpOpponent,int nbrCardBOT2s,int nbrCardBOT2sOpponent,int PlayerBOT2Number,int TOUR){
    

    double score;

		if (this.getCardBOT2Type() == 2){
    ...
    if (this.breaktrough){
      ...
      score3y += 10;
		...
		//cas des y //
		if (nbrCardBOT2s*2 <= nbrCardBOT2sOpponent && nbrCardBOT2sOpponent >=4){
			if (hp > 20){
				if (hpOpponent > 20){
			...
			if (hp > 20){
				if (hpOpponent > 23){
					...
					if (this.lethal && this.cost <4){
						if (PlayerBOT2Number == 1){score3x += 100;}
						else {score3x += 100;}
	...
	public String printATTACK(CardBOT2 c){
		String answer = "ATTACK"+" "+Integer.toString(this.getId())+" "+Integer.toString(c.getId())+";";
		//System.out.println(answer);
		return answer;
	}

  public String printATTACKBIS(CardBOT2 c){
		String answer = "ATTACK"+" "+Integer.toString(this.getId())+" "+Integer.toString(c.getId())+";";
		//System.out.println(answer);
		return answer;
	}
	...
	public String printUSE(CardBOT2 c){
	...
	public int getCardBOT2Number(){
		return this.idCardBOT2;
	}
	...
	public int getCardBOT2Type(){
		return this.CardBOT2Type;
	}
	...
	public void counterAttack(CardBOT2 c,JoueurBOT2 me,JoueurBOT2 other){
	...
	public void deCounterAttack(CardBOT2 c,JoueurBOT2 me,JoueurBOT2 other){
	...
	public void getAttackBy(CardBOT2 c,JoueurBOT2 me,JoueurBOT2 other,CardBOT2 opponent,CardBOT2 mine){
	...
	public void getDeattackBy(CardBOT2 c,JoueurBOT2 me,JoueurBOT2 other,CardBOT2 opponent,CardBOT2 mine){
	...
	public void getBuffByGreenCardBOT2(CardBOT2 c){
	...
	public void getDebuffByGreenCardBOT2(CardBOT2 c){
		this.defense -= c.getDefense();
		this.attack -= c.getAttack();
		if (c.getGuard()){
	...
	public void getBuffByGreenCardBOT2Bis(CardBOT2 c){
	...
	public void getBuffByRedCardBOT2(CardBOT2 c){
		if (this.ward == false){
			this.defense += c.getDefense();
			this.attack += c.getAttack();
		}
	...
	public void getDebuffByRedCardBOT2(CardBOT2 c){
	...
	public void getAttackByBlueCardBOT2(CardBOT2 c){
		this.defense += c.getDefense();
		this.attack += c.getAttack();
...
class HandBOT2{
	...
	private int amountOfCardBOT2;
	private Vector<CardBOT2> HandBOT2;
	private int amountOfItem;

	public HandBOT2(Vector<CardBOT2> h ) {
		this.HandBOT2 = h;
		this.amountOfCardBOT2 = h.size();
		this.amountOfItem = 0;
		for (int i = 0;i<h.size();i++) {
			if (h.elementAt(i).getCardBOT2Type() >0){this.amountOfItem++;}
		}
	}
	...
	public boolean equalWithPermutation(Vector<CardBOT2> vA,Vector<CardBOT2> vB){
		
		if (vA.size() != vB.size()){
			return false;
		} else {
			for (int i = 0 ; i < vA.size() ; i++){
				CardBOT2 test = vB.elementAt(i);
				if (test.getCardBOT2Type() > 0){return false;}
	...
	public boolean insideNoMatterpermutation(Vector<Vector<CardBOT2>> possible,Vector<CardBOT2> vB){
		for (int i = 0 ; i<possible.size() ; i++){
			Vector<CardBOT2> vA = possible.elementAt(i);
	...
	public Vector<CardBOT2> summonPossible(int mana,int nbrCarte,JoueurBOT2 me){
		Vector<CardBOT2> possible = new Vector<CardBOT2>();
		Vector<CardBOT2> h = this.HandBOT2;
	...
	public CardBOT2 CardBOT2RandomPossible(int mana,int nbrCarte,JoueurBOT2 me){
		Vector<CardBOT2> possible = this.summonPossible(mana, nbrCarte, me);
		if (possible.size() > 0){
			int randomIndice = ThreadLocalRandom.current().nextInt(0,possible.size());
			return possible.elementAt(randomIndice);
		} else {
			return new CardBOT2(0);
		}
		
	}

	public Vector<CardBOT2> randomSummon(int mana,int nbrCarte,JoueurBOT2 me){
		Vector<CardBOT2> Solution = new Vector<CardBOT2>();
		int m = mana;
		int compteur = nbrCarte;
		CardBOT2 toSummon;
		do {
			toSummon = this.CardBOT2RandomPossible(m, compteur, me);
			if (toSummon.getId() > -1){
				Solution.addElement(toSummon);
				m -= toSummon.getCost();
				if (toSummon.getCardBOT2Type() == 0){compteur++;}
				me.getHandBOT2().remove(toSummon);
			}
		} while (toSummon.getId()>-1 && compteur <= 6 && m >= 0);

		for (int i = 0 ; i < Solution.size();i++){
			if (Solution.elementAt(i).getId()>-1){me.getHandBOT2().add(Solution.elementAt(i));}
		}
		return Solution;
	}

	public Vector<Vector<CardBOT2>> allPosibilitesWithoutItem(int mana,int nbrCarte,JoueurBOT2 me,JoueurBOT2 other) {


		int min = 13;
		
		Vector<CardBOT2> h = this.HandBOT2;
		CardBOT2 minCost = new CardBOT2(0);
    ...
    Vector<CardBOT2> possible2 = new Vector<CardBOT2>();
		...
		Vector<Vector<CardBOT2>> renvoie = new Vector<Vector<CardBOT2>>();
		int n = possible.size();
		for (int i = 0;i<Math.pow(2,n);i++){
			int compteur = nbrCarte;
			int m = 0;
			Vector<CardBOT2> answer = new Vector<CardBOT2>();
			
			for (int j = 0;j<n;j++){
				if ((int)(i/(int)(Math.pow(2,n-1-j)))%2 == 0){
					if (possible.elementAt(j).getCardBOT2Type() == 0|| possible.elementAt(j).getCardBOT2Type() == 3 || (possible.elementAt(j).getCardBOT2Type() == 1 && compteur>0) || (possible.elementAt(j).getCardBOT2Type() == 2 && other.getBoard().size()>1)){
						answer.addElement(possible.elementAt(j));
						m += possible.elementAt(j).getCost();
						if (possible.elementAt(j).getCardBOT2Type() == 0){compteur++;}
					}
				}
			}
    ...
    for (int i = 0;i<Math.pow(2,n);i++){
			...
			for (int j = 0;j<n;j++){
				if ((int)(i/(int)(Math.pow(2,n-1-j)))%2 == 0){
					if (possible2.elementAt(j).getCardBOT2Type() == 0|| possible2.elementAt(j).getCardBOT2Type() == 3 || (possible2.elementAt(j).getCardBOT2Type() == 1 && compteur>0) || (possible2.elementAt(j).getCardBOT2Type() == 2 && other.getBoard().size()>1)){
						answer.addElement(possible2.elementAt(j));
						m += possible2.elementAt(j).getCost();
						if (possible2.elementAt(j).getCardBOT2Type() == 0){compteur++;}
					}
				}
			}
...
class DeckBOT2{
	...
	int amountOfCardBOT2;
	Vector<CardBOT2> DeckBOT2;
	int[] manaAmount;
	int amountOfItem;
	int[] scoreCardBOT22 = {73
		,69
		,87
		,72
		,80
...
//	{18, 96, 87, 81, 89, 10, 129, 83, 92, 62, 123, 78, 42, 54, 79, 46, 79, 95, 50, 105, 115, 69, 19, 70, 88, 85, 54, 100, 100, 70, 59, 99, 30, 24, 65, 80, 4, 4, 77, 127, 100, 41, 52, 88, 55, 10, 56, 71, 41, 99, 112, 102, 28, 97, 22, 70, 52, 53, 55, 46, 68, 61, 71, 83, 65, 74, 84, 98, 111, 79, 82, 74, 70, 57, 71, 50, 63, 36, 66, 78, 72, 116, 100, 119, 96, 89, 92, 91, 67, 64, 91, 71, 81, 70, 94, 87, 35, 77, 88, 72, 67, 61, 81, 75, 68, 90, 45, 52, 76, 13, 21, 59, 33, 64, 73,114,38,59,52,54,76,62,44,43,58,60,64,58,68,54,47,48,74,63,64,47,71,35,99,4,80,52,16,85,87,68,92,94,75,85,121,83,10,9,0,5,9,0,5,4};
	...
	public DeckBOT2(){
		this.amountOfCardBOT2 = 0;
		this.DeckBOT2 = new Vector<CardBOT2>();
	...
	public int getAmountOfCardBOT2(){
		return this.amountOfCardBOT2;
	}

	public Vector<CardBOT2> getDeckBOT2(){
		return this.DeckBOT2;
	}
	...
	public void incrCardBOT2(CardBOT2 c){
		this.amountOfCardBOT2++;
		this.DeckBOT2.addElement(c);
		if (c.getCardBOT2Type() > 0){this.amountOfItem++;}
		else{
	...
	public void decrCardBOT2(CardBOT2 c){
		this.amountOfCardBOT2--;
		this.DeckBOT2.removeElementAt(this.DeckBOT2.size()-1);
		if (c.getCardBOT2Type() > 0){this.amountOfItem--;}
		else{
			this.manaAmount[c.getCost()]--;
		}
	...
	public int scoreCardBOT2InDraft(CardBOT2 c,int n){
		int indx = c.getCardBOT2Number();
		int score;
		if (n == 1){
			score = this.scoreCardBOT2[indx-1];
		}
		else {
			score =  this.scoreCardBOT22[indx-1];
		}
	...
	public int scoreDeckBOT2(){
		...
		score *= this.amountOfCardBOT2;

		CardBOT2 c = this.DeckBOT2.elementAt(this.DeckBOT2.size()-1);
...
class JoueurBOT2{
	...
	private int hp;
	private int mana;
	private int remainingCardBOT2sInDeckBOT2;
	private int rune;
	private Vector<CardBOT2> board;
	private Vector<CardBOT2> HandBOT2;


	public JoueurBOT2(int hp,int mana,int rcid,int rune,Vector<CardBOT2> HandBOT2, Vector<CardBOT2> board){
		this.hp = hp;
		this.mana = mana;
		this.remainingCardBOT2sInDeckBOT2 = rcid;
		this.rune = rune;
		this.board = board;
		this.HandBOT2 = HandBOT2;
	}

  public boolean validAnswerBis(Vector<Vector<CardBOT2>> a,JoueurBOT2 opponent,CardBOT2 other,CardBOT2 mine){
	  	int kill = 0;
		boolean answer = true;
		Vector<CardBOT2> opponentBoard = new Vector<CardBOT2>();
		Vector <CardBOT2> b = opponent.getBoard();
		Vector<CardBOT2> gInBoard = opponent.getGuardsInBoard();
		Vector<CardBOT2> attacked = new Vector<CardBOT2>();
		int let = 0;
		int amountOfLethal = this.getLethalsInBoard().size();
		//for (int i = 0;i<b.size();i++){opponentBoard.addElement(b.elementAt(i));}
		for (int i = 0;i<a.size();i++){
			CardBOT2 attacker = a.elementAt(i).elementAt(0);
			CardBOT2 defender = a.elementAt(i).elementAt(1);
			int n = gInBoard.size();
	...
	public boolean validAnswerBisOpponent(Vector<Vector<CardBOT2>> a,JoueurBOT2 opponent,CardBOT2 other,CardBOT2 mine){
		int kill = 0;
	  boolean answer = true;
	  Vector<CardBOT2> opponentBoard = new Vector<CardBOT2>();
	  Vector <CardBOT2> b = opponent.getBoard();
  ...
  public void setHandBOT2(Vector<CardBOT2> HandBOT2){
		this.HandBOT2 = HandBOT2;
	}


	public void setBoard(Vector<CardBOT2> board){
		this.HandBOT2 = board;
	}

	public void clean(){
		for (int i =1;i<this.getBoard().size();i++ ){
			CardBOT2 c = this.getBoard().elementAt(i);
			if (c.getId() == -1){
	...
	public Vector<CardBOT2> boardOrderByAttack(){
		Vector<CardBOT2> order = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		while (b.size()>0){
			int bestScore = -1;
			CardBOT2 best = b.elementAt(0);
			for (int i = 0;i<b.size();i++){
				int score = 0;
				CardBOT2 c = b.elementAt(i);
				if (c.getLethal() == true){
					score += 99;
				}
				score += c.getAttack();
	...
	public boolean hasEmperorNightmareInBoard(){
		for (int i = 0 ; i < this.board.size() ; i++){
			if (this.board.elementAt(i).getCardBOT2Number() == 116){
				return true;
			}
		}
		return false;
	...
	public boolean hasEmperorNightmareInHandBOT2(){
		for (int i = 0 ; i < this.HandBOT2.size() ; i++){
			if (this.HandBOT2.elementAt(i).getCardBOT2Number() == 116){
				return true;
			}
		}
		return false;
	...
	public void removeEmperorFromHandBOT2(){
		for (int i = 0 ; i < this.HandBOT2.size() ; i++){
			if (this.HandBOT2.elementAt(i).getCardBOT2Number() == 116){
				this.HandBOT2.remove(this.HandBOT2.elementAt(i));
			}
		}
	...
	public Vector<CardBOT2> getGuardsInBoardOrderByDefense(){
		Vector<CardBOT2> order = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getGuardsInBoard();
		while (b.size()>0){
	...
	public Vector<CardBOT2> boardOrderByDefense(){
		Vector<CardBOT2> order = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		while (b.size()>0){
			int bestScore = -1000000;
	...
	public int getRemainingCardBOT2sInDeckBOT2(){
		return this.remainingCardBOT2sInDeckBOT2;
	}
	...
	public Vector<CardBOT2> getBoard(){
		return this.board;
	}

	public Vector<CardBOT2> getHandBOT2(){
		return this.HandBOT2;
	}

	public void getAttackByCardBOT2(CardBOT2 c){
		this.hp -= c.getAttack();
	}

	public void getBuffByBlueCardBOT2(CardBOT2 c){
	...
	public Vector<CardBOT2> getGuardsInBoard(){
		Vector<CardBOT2> g = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		for (int i = 0;i<b.size();i++){
	...
	public Vector<CardBOT2> getWardsInBoard(){
		Vector<CardBOT2> g = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		for (int i = 0;i<b.size();i++){
	...
	public Vector<CardBOT2> getDrainsInBoard(){
		Vector<CardBOT2> g = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		for (int i = 0;i<b.size();i++){
	...
	public Vector<CardBOT2> getChargesInBoard(){
		Vector<CardBOT2> g = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		for (int i = 0;i<b.size();i++){
	...
	public Vector<CardBOT2> getBreaktroughsInBoard(){
		Vector<CardBOT2> g = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		for (int i = 0;i<b.size();i++){
	...
	public Vector<CardBOT2> getLethalsInBoard(){
		Vector<CardBOT2> g = new Vector<CardBOT2>();
		Vector<CardBOT2> b = this.getBoard();
		for (int i = 0;i<b.size();i++){
...
class TurnBOT2{
	...
	private int tour;
	private JoueurBOT2 me;
	private JoueurBOT2 other;


	public TurnBOT2(int tour,JoueurBOT2 me, JoueurBOT2 other){
		this.tour = tour;
		this.me = me;
		this.other = other;
	}
  ...
  public void incrTurnBOT2(){
		this.tour++;
	}

	public void setTurnBOT2(int TurnBOT2){
		this.tour = TurnBOT2;
	}

	public int getTurnBOT2(){
		return this.tour;
	}

	public JoueurBOT2 getMe(){
		return this.me;
	}

	public JoueurBOT2 getOther(){
		return this.other;
	}

	public void afficher(){
		////System.err.println("Le tour est constitué de ");
		Vector<CardBOT2> myBoard = this.me.getBoard();
		Vector<CardBOT2> hisBoard = this.other.getBoard();

		////System.err.println("Mon board ");
		for (int i = 0;i<myBoard.size();i++){
			CardBOT2 c = myBoard.elementAt(i);
		...
		for (int i = 0;i<hisBoard.size();i++){
			CardBOT2 c = hisBoard.elementAt(i);
			////System.err.println("Carte "+c.getId()+" d'attaque "+c.getAttack()+" de defense "+c.getDefense()+" d'abilities "+c.getAbilities());
		}
	}
  ...
  public double getMyTempo(CardBOT2 opponent,CardBOT2 mine){
	if (opponent.getDefense() <= 0){return (double)100000.0;}
	Vector<CardBOT2> g = this.other.getBoard();
	...
	Vector<CardBOT2> b = this.me.getBoard();
	...
	Vector<CardBOT2> w = this.other.getWardsInBoard();
	Vector<CardBOT2> l = this.me.getLethalsInBoard();

	lifeOpponent += (double)w.size()*avgAttack;
	//lifeOpponent -= (double)l.size()*(bestDefense);
	
	Vector<CardBOT2> b1 = this.me.getBreaktroughsInBoard();
	myAttack += (double) b1.size();
	Vector<CardBOT2> d = this.me.getDrainsInBoard();
...
public double getHisTempo(CardBOT2 opponent,CardBOT2 mine){
	if (mine.getDefense() <= 0){return (double)10000.0;}
	Vector<CardBOT2> g = this.me.getBoard();
	...
	Vector<CardBOT2> b = this.other.getBoard();
	double myAttack = (double)0;
	for (int i = 0;i<b.size();i++){
		if (b.elementAt(i).getDefense()>0) {myAttack += (double) Math.max(0,b.elementAt(i).getAttack());}
	}
	...
	Vector<CardBOT2> w = this.me.getWardsInBoard();
	Vector<CardBOT2> l = this.other.getLethalsInBoard();

	lifeOpponent += (double)w.size()*avgAttack;
	lifeOpponent -= (double)l.size()*(bestDefense);
	
	Vector<CardBOT2> b1 = this.other.getBreaktroughsInBoard();
	myAttack += (double) b1.size();
	Vector<CardBOT2> d = this.other.getDrainsInBoard();
	double tempTempo = (double)(lifeOpponent+0.1)/(myAttack+0.1);
	for (int i = 0;i<d.size();i++){
		myAttack += (tempTempo/(double)2.0) * (double)d.elementAt(i).getAttack();
	}
...
public double getTurnBOT2Score(CardBOT2 opponent,CardBOT2 mine){
	...
	int deltaHandBOT2 = this.me.getHandBOT2().size() - this.other.getHandBOT2().size() - 1;
	int deltaBoard = 0;
	...
	if (deltaHandBOT2 + deltaBoard - runeDropped < 0){return myScore - hisScore-50;}
	else {return myScore - hisScore;}
	
}

  
	public double getMyBoardScore(CardBOT2 opponent,CardBOT2 mine,int PlayerBOT2Number,int TOUR){
	    
		int hp = mine.getDefense();
		int hpOpponent = opponent.getDefense();
		if (hpOpponent <= 0){return (double)100000.0;}

		int nbrCardBOT2s = this.me.getBoard().size();
		int nbrCardBOT2sOpponent = this.other.getBoard().size();
		double score = (double)hp;
    //////System.err.println("nbr mes CardBOT2s : "+nbrCardBOT2s);
		for (int i = 0;i<nbrCardBOT2s;i++){
      if (this.me.getBoard().elementAt(i).getAttack() == 0 && this.me.getBoard().elementAt(i).getGuard() == false){
				score -= 300;
			}
			if(this.me.getBoard().elementAt(i).getId()>-1 && this.me.getBoard().elementAt(i).getDefense() > 0){ score += this.me.getBoard().elementAt(i).scoreBis(hp,hpOpponent,nbrCardBOT2s,nbrCardBOT2sOpponent,PlayerBOT2Number,TOUR);}
		}
	...
	public double getHisBoardScore(CardBOT2 opponent,CardBOT2 mine,int PlayerBOT2Number,int TOUR){
    ...
    //////System.err.println("nbr ses CardBOT2s : "+nbrCardBOT2sOpponent);

		for (int i = 1;i<nbrCardBOT2sOpponent;i++){
      		if (this.other.getBoard().elementAt(i).getAttack() == 0 && this.other.getBoard().elementAt(i).getGuard() == false){
				score -= 300.0;
			}
			if (this.other.getBoard().elementAt(i).getId()>-1 && this.other.getBoard().elementAt(i).getDefense() > 0){score += (double)this.other.getBoard().elementAt(i).scoreBis(hpOpponent,hp,nbrCardBOT2sOpponent,nbrCardBOT2s,PlayerBOT2Number,TOUR);}
	...
	public double getTurnBOT2ScoreSummon(CardBOT2 opponent,CardBOT2 mine,int PlayerBOT2Number,int TOUR){
		double myScore = this.getMyBoardScore(opponent,mine,PlayerBOT2Number,TOUR);
		double hisScore =0;// this.getHisBoardScore(opponent,mine,PlayerBOT2Number,TOUR);
		return myScore - hisScore;
	}

	
  public CardBOT2 bestUseOfGreenCardBOT2(CardBOT2 greenItem,CardBOT2 opponent,CardBOT2 mine,Vector<CardBOT2> summonWithoutCharge){
		
		Vector<CardBOT2> b = new Vector<CardBOT2>();
  		b.addAll(this.me.getBoard());
		b.addAll(summonWithoutCharge);
		CardBOT2 bestAnswer = new CardBOT2(0);
		if (b.size() > 0){
	...
	public CardBOT2 bestUseOfRedCardBOT2(CardBOT2 redItem,CardBOT2 opponent,CardBOT2 mine){
		Vector<CardBOT2> b = this.other.getBoard();
		if (redItem.getCardBOT2Number() == 142 || redItem.getCardBOT2Number() == 149){
			Vector<CardBOT2> possible = new Vector<CardBOT2>();
			for (int i = 0 ; i < b.size() ; i++){
			...
			b = possible;
		} else if (redItem.getCardBOT2Number() == 143){
			Vector<CardBOT2> possible = new Vector<CardBOT2>();
			for (int i = 0 ; i < b.size() ; i++){
				if (b.elementAt(i).getGuard()){
					possible.add(b.elementAt(i));
	...
	public Vector<Vector<CardBOT2>> generateRandomAttack(int compteur,long startTime,CardBOT2 opponent, CardBOT2 mine){


		int n = this.me.getBoard().size();
		Vector<CardBOT2> hisBoard = new Vector<CardBOT2>();
		Vector<CardBOT2> hisGuard = new Vector<CardBOT2>();
		...
		Vector<CardBOT2> damaged = new Vector<CardBOT2>();
		...
		Vector<Vector<CardBOT2>> renvoie = new Vector<Vector<CardBOT2>>();
			...
			while (indice.size() > n - sizeAttack){
				...
				CardBOT2 attacker = this.me.getBoard().elementAt(indiceAttack);
				int indiceDef;
				CardBOT2 defender;
				...
				Vector<CardBOT2> att = new Vector<CardBOT2>();
...
public Vector<Vector<CardBOT2>> generateRandomAttackOpponent(int compteur,long startTime,CardBOT2 opponent, CardBOT2 mine){
	...
	Vector<Vector<CardBOT2>> renvoie = new Vector<Vector<CardBOT2>>();
		...
		while (indice.size() > n - sizeAttack){
			int randomNum = ThreadLocalRandom.current().nextInt(0, indice.size());
			int indiceAttack = indice.elementAt(randomNum);
			CardBOT2 attacker = this.other.getBoard().elementAt(indiceAttack);
			int indiceDef;
			CardBOT2 defender;
...
public Vector<Vector<CardBOT2>> bestRandomAttackOpponent(CardBOT2 opponent,CardBOT2 mine){
	...
	Vector<Vector<CardBOT2>> bestAnswer = new Vector<Vector<CardBOT2>>();
	long endTimeOpponent = System.nanoTime();
	Vector<Vector<Vector<CardBOT2>>> tested = new Vector<Vector<Vector<CardBOT2>>>();
	...
	while ((endTimeOpponent - startTimeOpponent) <300000 && iteration < 10){
		...
		Vector<Vector<CardBOT2>> attaque = this.generateRandomAttackOpponent(0,startTimeOpponent,opponent,mine);
		
		//if (!(tested.contains(attaque)) && this.other.validAnswerBis(attaque,this.me,mine,opponent)){
		if (!(tested.contains(attaque))){
			Vector<CardBOT2> myDead = new Vector<CardBOT2>();
			Vector<CardBOT2> hisDead = new Vector<CardBOT2>();
		...
		for (int m = 0 ; m < this.me.getBoard().size();m++){
			CardBOT2 c = this.me.getBoard().elementAt(m);
		...
		for (int m = 0 ; m < this.other.getBoard().size();m++){
			CardBOT2 c = this.other.getBoard().elementAt(m);
			if (c.getDefense() >0 && c.getId() > -1){
				//scoreBis--;
				score++;
		...
		if (mine.getDefense()<=0){
			...
			//double tempScoreSecond = getTurnBOT2Score(opponent,mine);//this.getTurnBOT2Score(opponent, mine);
			//double tempScoreSecond = mine.getDefense() - opponent.getDefense();
			//double scoreBis = this.getTurnBOT2Score(opponent, mine);
			if (score > bestScore){
				bestScore = score;
				bestAnswer = attaque;
			}
			...
			/*for (int j = hisNextAttack.size()-1;j>=0;j--){
				CardBOT2 attacker = hisNextAttack.elementAt(j).elementAt(0);
				CardBOT2 defender = hisNextAttack.elementAt(j).elementAt(1);
			...
			for (int i = attaque.size()-1;i >= 0;i--){
				CardBOT2 attacker = attaque.elementAt(i).elementAt(0);
				CardBOT2 defender = attaque.elementAt(i).elementAt(1);
				if (defender.getDefense() <= 0 && defender.getId() > -1){
...
public boolean areEqual(double a,double b){
	...
	public Vector<Vector<CardBOT2>> attackBasicSolutionDirect(CardBOT2 opponent,CardBOT2 mine){
		Vector<Vector<CardBOT2>> renvoie = new Vector<Vector<CardBOT2>>();
		Vector<CardBOT2> b = this.other.boardOrderByAttack();
		Vector<CardBOT2> bOpponent = this.me.getBoard();
		Vector<CardBOT2> g = this.me.getGuardsInBoard();
		...
		for (int i = 0 ; i < renvoie.size() ; i++){
			CardBOT2 attacker = renvoie.elementAt(i).elementAt(0);
			CardBOT2 defender = renvoie.elementAt(i).elementAt(1);
			attacker.resetSmall();
			defender.resetSmall();
	...
	public Vector<Vector<CardBOT2>> attackDirecte(CardBOT2 opponent,CardBOT2 mine){
		Vector<Vector<CardBOT2>> renvoie = new Vector<Vector<CardBOT2>>();
		Vector<CardBOT2> b = this.me.boardOrderByAttack();
		Vector<CardBOT2> bOpponent = this.other.getBoard();
		Vector<CardBOT2> g = this.other.getGuardsInBoard();
		Vector<CardBOT2> attacked = new Vector<CardBOT2>();
		for (int i = 0;i<b.size();i++){
...
public void updateItem(Vector<CardBOT2> tempUseGreenItem,Vector<CardBOT2> tempUseRedItem){
...
public void summonThings(Vector<CardBOT2> summoning,Vector<CardBOT2> summonWithoutCharge,CardBOT2 opponent, CardBOT2 mine,Vector<CardBOT2> tempGreenItem,Vector<CardBOT2> tempRedItem,Vector<CardBOT2> tempUseGreenItem,Vector<CardBOT2> tempUseRedItem){
	
	if (summoning.size() > 0){
		////System.err.print("summoning : ");
		for (int j = 0;j<summoning.size();j++){
			CardBOT2 c = summoning.elementAt(j);
			
			////System.err.print(" ; "+c.getId()+" ; ");
			if (c.getCardBOT2Type() < 1){
			...
			else if (c.getCardBOT2Type() == 1 && this.me.getBoard().size()>0){
				CardBOT2 bestUse = this.bestUseOfGreenCardBOT2(c, opponent, mine,summonWithoutCharge);
				if (bestUse.getId() >-1){
					
					tempUseGreenItem.addElement(bestUse);
					tempGreenItem.addElement(c);
					bestUse.getBuffByGreenCardBOT2(c);
					//////System.err.print("USE"+" "+c.getId()+" "+bestUse.getId()+" ; ");
				}
				
			}
			else if (c.getCardBOT2Type() == 2 && this.other.getBoard().size()>1){
				CardBOT2 bestUse = bestUseOfRedCardBOT2(c, opponent, mine);
				if (bestUse.getId()>-1){
					
					tempUseRedItem.addElement(bestUse);
					tempRedItem.addElement(c);
					//////System.err.println("USE"+" "+c.getId()+" "+bestUse.getId()+" ; ");
					bestUse.getBuffByRedCardBOT2(c);
...
public void unSummonThings(Vector<CardBOT2> summoning,Vector<CardBOT2> summonWithoutCharge,CardBOT2 opponent, CardBOT2 mine,Vector<CardBOT2> tempGreenItem,Vector<CardBOT2> tempRedItem,Vector<CardBOT2> tempUseGreenItem,Vector<CardBOT2> tempUseRedItem){
	for (int j = summoning.size()-1;j>=0;j--){
		...
		if (c.getCardBOT2Type() < 1){
			...
			//CardBOT2 reverseC = new CardBOT2(c);
	...
	for (int m = 0;m<tempGreenItem.size();m++){
		CardBOT2 objet = tempGreenItem.elementAt(m);
		CardBOT2 bestUse = tempUseGreenItem.elementAt(m);
		bestUse.getDebuffByGreenCardBOT2(objet);
	}

	for (int m = 0 ; m<tempRedItem.size();m++){
		CardBOT2 objet = tempRedItem.elementAt(m);
		CardBOT2 bestUse = tempUseRedItem.elementAt(m);
		if (bestUse.getDefense()<=0 && !this.other.getBoard().contains(bestUse)){
			this.other.getBoard().add(bestUse);
		}
		bestUse.getDebuffByRedCardBOT2(objet);		
	}
}

public void myAttack(Vector<Vector<CardBOT2>> attaque,CardBOT2 opponent,CardBOT2 mine,Vector<CardBOT2> myDead,Vector<CardBOT2> hisDead){
		////System.err.println("");
		////System.err.print("Mon attaque ");
		for (int j = 0;j<attaque.size();j++){
			CardBOT2 attacker = attaque.elementAt(j).elementAt(0);
			CardBOT2 defender = attaque.elementAt(j).elementAt(1);
...
public void hisAttack(Vector<Vector<CardBOT2>> hisNextAttack,CardBOT2 opponent,CardBOT2 mine,Vector<CardBOT2> myDead,Vector<CardBOT2> hisDead){
		////System.err.println("");
...
public void update(Vector<CardBOT2> summonWithoutCharge){
...
public void downgrade(Vector<CardBOT2> summonWithoutCharge,Vector<CardBOT2> myDead,Vector<CardBOT2> hisDead){
...
public void resetFull(Vector<CardBOT2> summonWithoutCharge){
	for (int m = 0 ; m < this.me.getBoard().size();m++){
		CardBOT2 c = this.me.getBoard().elementAt(m);
		c.reset();
	}
	for (int m = 0 ; m < this.me.getHandBOT2().size();m++){
		CardBOT2 c = this.me.getHandBOT2().elementAt(m);
		c.reset();
	}
	for (int m = 0 ; m < summonWithoutCharge.size();m++){
		CardBOT2 c = summonWithoutCharge.elementAt(m);
		c.reset();
	}
	for (int m = 0 ; m < this.other.getBoard().size();m++){
		CardBOT2 c = this.other.getBoard().elementAt(m);
...
public String bestAttackRandomDepthSummon(String a,long startTime,Vector<CardBOT2> summonWithoutCharge,CardBOT2 mine){
	int mana = this.me.getMana();
	HandBOT2 HandBOT2 = new HandBOT2(this.me.getHandBOT2());
	int nbrCarte = this.me.getBoard().size();
	int compteur = 0;
	int compteurGlobal = 0;
	Vector<CardBOT2> bestSummon = new Vector<CardBOT2>();
	Vector<Vector<CardBOT2>> bestAnswer = new Vector<Vector<CardBOT2>>();
  	CardBOT2 opponent = this.other.getBoard().elementAt(0);
	//double bestScore = -10000000.0;//this.getTurnBOT2Score(opponent,mine);
	//double bestScoreSecond = -1000000.0;
	//int bestScoreBis = - 100000;  
	int bestScoreBis = -10000;
	//init initScore = -1000000;////this.getTurnBOT2Score(opponent,mine);
	int bestScore = -100;
	double bestScoreSecond = - 100000;  
	Vector<CardBOT2> bestGreenItem = new Vector<CardBOT2>();
	Vector<CardBOT2> bestRedItem = new Vector<CardBOT2>();
	Vector<CardBOT2> bestUseGreenItem = new Vector<CardBOT2>();
	Vector<CardBOT2> bestUseRedItem = new Vector<CardBOT2>();
	long endTime = System.nanoTime();
	

	while ((endTime - startTime) <95*1000000){
		Vector<CardBOT2> summoning = HandBOT2.randomSummon(mana, nbrCarte,this.me);
		Vector<CardBOT2> tempGreenItem = new Vector<CardBOT2>();
		Vector<CardBOT2> tempRedItem = new Vector<CardBOT2>();
		Vector<CardBOT2> tempUseGreenItem = new Vector<CardBOT2>();
		Vector<CardBOT2> tempUseRedItem = new Vector<CardBOT2>();
		Vector<CardBOT2> myDead = new Vector<CardBOT2>();
		Vector<CardBOT2> hisDead = new Vector<CardBOT2>();
		Vector<CardBOT2> hugeSummon = new Vector<CardBOT2>();

		for (int i = 0 ; i < summoning.size() ; i++){
			if (summoning.elementAt(i).getCost() >=7 && summoning.elementAt(i).getCardBOT2Type() == 0 && summoning.elementAt(i).getCharge() == false){
		...
		Vector<Vector<CardBOT2>> attaque = new Vector<Vector<CardBOT2>>();
		boolean test;
		...
		//double testScore = this.getTurnBOT2Score(opponent,mine);
//testScore >= bestScore
		if (true){
		compteur++;
        ...
        Vector<Vector<CardBOT2>> hisNextAttack =this.bestRandomAttackOpponent(opponent,mine);
		...
		endTime = System.nanoTime();
			...
			Vector<CardBOT2> items = new Vector<CardBOT2>();
			Vector<CardBOT2> finalAnswer = bestSummon;
			for (int i = 0;i<finalAnswer.size();i++){
				CardBOT2 c = finalAnswer.elementAt(i);
				if (c.getCardBOT2Type() == 0){
					a += c.printSUMMON();
					...
					this.me.getHandBOT2().removeElement(c);
			...
			for (int m = 0;m<bestGreenItem.size();m++){
				CardBOT2 objet = bestGreenItem.elementAt(m);
				CardBOT2 bestUse = bestUseGreenItem.elementAt(m);
				this.me.getHandBOT2().removeElement(objet);
				bestUse.getBuffByGreenCardBOT2Bis(objet);
				a += bestUse.printUSE(objet);
				this.me.setMana(this.me.getMana()-objet.getCost());
			}
			for (int m = 0 ; m<bestRedItem.size();m++){
				CardBOT2 objet = bestRedItem.elementAt(m);
				CardBOT2 bestUse = bestUseRedItem.elementAt(m);
				bestUse.getBuffByRedCardBOT2(objet);
				this.me.getHandBOT2().removeElement(objet);
				a += bestUse.printUSE(objet);
				this.me.setMana(this.me.getMana()-objet.getCost());
			}
			...
			for (int i = 0;i<bestAnswer.size();i++){
				CardBOT2 attacker = bestAnswer.elementAt(i).elementAt(0);
				CardBOT2 defender = bestAnswer.elementAt(i).elementAt(1);
				defender.getAttackBy(attacker,this.me,this.other,opponent,mine);
			defender.counterAttack(attacker,this.me,this.other);
  			if (defender.getDefense() <=0){
				this.other.getBoard().removeElement(defender);
...
public String summonPossibilities(String a,Vector<CardBOT2> summonWithoutCharge,CardBOT2 opponent,CardBOT2 mine,int PlayerBOT2Number,int TOUR){

	HandBOT2 HandBOT2 = new HandBOT2(this.me.getHandBOT2());
	int mana = this.me.getMana();
	

		Vector<Vector<CardBOT2>> possible = HandBOT2.allPosibilitesWithoutItem(mana,this.me.getBoard().size(),this.me,this.other);
		if (possible.size() >0){
	...
	Vector<CardBOT2> bestUseRedItem = new Vector<CardBOT2>();
		...
		for (int i = 0;i<possible.size();i++){
			double score = 0;
			Vector<CardBOT2> summoning = possible.elementAt(i);
	//////System.err.println("");
			int manaUsed = 0;

			Vector<CardBOT2> bestUseGreen = new Vector<CardBOT2>();
			Vector<CardBOT2> bestUseRed = new Vector<CardBOT2>();
			Vector<CardBOT2> tempGreenItem = new Vector<CardBOT2>();
	Vector<CardBOT2> tempRedItem = new Vector<CardBOT2>();
	Vector<CardBOT2> tempUseGreenItem = new Vector<CardBOT2>();
	Vector<CardBOT2> tempUseRedItem = new Vector<CardBOT2>();
	...
	for (int j = 0;j<summoning.size();j++){
		...
		else if (c.getCardBOT2Type() == 1 && this.me.getBoard().size()+summonWithoutCharge.size()>0){
			CardBOT2 bestUse = this.bestUseOfGreenCardBOT2(c, opponent, mine,summonWithoutCharge);
			if (bestUse.getId() >-1){
				manaUsed += c.getCost();
				tempUseGreenItem.addElement(bestUse);
...
for (int me = 0;me<summonWithoutCharge.size();me++){
	this.me.getBoard().add(summonWithoutCharge.elementAt(me));
			...
			score = getTurnBOT2ScoreSummon(opponent,mine,1,30);//this.getTurnBOT2Score(opponent,mine);
	...
	public void pickDraft(DeckBOT2 d,int PlayerBOT2Number){
		if (this.getTurnBOT2() < 30){
			int score = -100000;
			int best = 0;

			Vector<CardBOT2> h = this.me.getHandBOT2();
			d.incrCardBOT2(h.elementAt(0));
			int score0 = d.scoreCardBOT2InDraft(h.elementAt(0),PlayerBOT2Number);
			d.decrCardBOT2(h.elementAt(0));
			d.incrCardBOT2(h.elementAt(1));
			int score1 = d.scoreCardBOT2InDraft(h.elementAt(1),PlayerBOT2Number);
			d.decrCardBOT2(h.elementAt(1));
			d.incrCardBOT2(h.elementAt(2));
			int score2 = d.scoreCardBOT2InDraft(h.elementAt(2),PlayerBOT2Number);
			d.decrCardBOT2(h.elementAt(2));
			...
			CardBOT2 bestCardBOT2 = h.elementAt(best);

            
			d.incrCardBOT2(bestCardBOT2);
			bestCardBOT2.printPICK();
  ...
  public String attackBasicBis(String a,long startTime,Vector<CardBOT2> summonWithoutCharge,CardBOT2 mine){
		CardBOT2 opponent = this.other.getBoard().elementAt(0);
		//Vector<Vector<CardBOT2>> answer = this.bestAttackRandom(startTime,summonWithoutCharge,mine);
		//Vector<Vector<CardBOT2>> answer =  bestRandomAttack(opponent,mine,summonWithoutCharge,0);
...
public static void main(String args[]) {
		int TOUR = 0;
		DeckBOT2 d = new DeckBOT2();
		int PlayerBOT2Number = 1;
		...
		while (true) {
				...
				int JoueurBOT2Health = in.nextInt();
				int JoueurBOT2Mana = in.nextInt();
				int JoueurBOT2DeckBOT2 = in.nextInt();
				int JoueurBOT2Rune = in.nextInt();
				//JoueurBOT2 JoueurBOT21 = new JoueurBOT2(JoueurBOT2Health,JoueurBOT2Mana,JoueurBOT2DeckBOT2,JoueurBOT2Rune);
				int JoueurBOT2HealthOpponent = in.nextInt();
				int JoueurBOT2ManaOpponent = in.nextInt();
				int JoueurBOT2DeckBOT2Opponent = in.nextInt();
				int JoueurBOT2RuneOpponent = in.nextInt();
				//JoueurBOT2 JoueurBOT22 = new JoueurBOT2(JoueurBOT2HealthOpponent,JoueurBOT2ManaOpponent,JoueurBOT2DeckBOT2Opponent,JoueurBOT2RuneOpponent);
				int opponentHandBOT2 = in.nextInt();
				Vector<CardBOT2> fake = new Vector<CardBOT2>();
				for (int o = 0;o<opponentHandBOT2;o++){
					CardBOT2 lol = new CardBOT2(0);
					fake.add(lol);
				}
				int CardBOT2Count = in.nextInt();
				Vector<CardBOT2> board = new Vector();
				Vector<CardBOT2> HandBOT2 = new Vector();
				Vector<CardBOT2> boardOpponent = new Vector();
        		Vector<CardBOT2> summonWithoutCharge = new Vector();
				CardBOT2 opponent = new CardBOT2(JoueurBOT2HealthOpponent);
        		CardBOT2 mine = new CardBOT2(JoueurBOT2Health);
				boardOpponent.addElement(opponent);
				summonWithoutCharge.addElement(mine);
				long startTime = System.nanoTime();
				for (int i = 0; i < CardBOT2Count; i++) {
						int CardBOT2Number = in.nextInt();
						

						int instanceId = in.nextInt();
						int location = in.nextInt();
						int CardBOT2Type = in.nextInt();
				
						int cost = in.nextInt();
						int attack = in.nextInt();
						int defense = in.nextInt();
						if (CardBOT2Type == 3 && defense != 0){
							CardBOT2Type = 2;
						}
						String abilities = in.next();
						int myHealthChange = in.nextInt();
						int opponentHealthChange = in.nextInt();
						int CardBOT2Draw = in.nextInt();
						CardBOT2 c = new CardBOT2(CardBOT2Number,instanceId,location,CardBOT2Type,cost,attack,defense,abilities,myHealthChange,opponentHealthChange,CardBOT2Draw,i);
						if (location == 0){
				...
				JoueurBOT2 JoueurBOT21 = new JoueurBOT2(JoueurBOT2Health,JoueurBOT2Mana,JoueurBOT2DeckBOT2,JoueurBOT2Rune,HandBOT2,board);
				JoueurBOT2 JoueurBOT22 = new JoueurBOT2(JoueurBOT2HealthOpponent,JoueurBOT2ManaOpponent,JoueurBOT2DeckBOT2Opponent,JoueurBOT2RuneOpponent,fake,boardOpponent);
				//JoueurBOT2 JoueurBOT22 = new JoueurBOT2(JoueurBOT2HealthOpponent,JoueurBOT2ManaOpponent,JoueurBOT2DeckBOT2Opponent,JoueurBOT2RuneOpponent,fake,hisGuards);

				//JoueurBOT21.setHandBOT2(HandBOT2);
				//JoueurBOT21.setBoard(board);
				//JoueurBOT22.setBoard(boardOpponent);
				TurnBOT2 tour = new TurnBOT2(TOUR,JoueurBOT21,JoueurBOT22);
        ...
        */
				...
				if (TOUR == 0){
					...
					if (JoueurBOT2DeckBOT2Opponent == 0){PlayerBOT2Number = 1;}
					if (JoueurBOT2DeckBOT2Opponent == 1){PlayerBOT2Number = 2;}
					////System.err.println(PlayerBOT2Number);
				}
				if (TOUR <30){tour.pickDraft(d,PlayerBOT2Number);}
				else{
				    String answer = "PASS;";
				String hisAnswer = "";
				/*int amountOfCrea = 0;
				int manaToUse = 0;
				for (int i = 0 ; i < tour.getMe().getHandBOT2().size() ; i++){
					if (tour.getMe().getHandBOT2().elementAt(i).getCardBOT2Type() == 0){
						amountOfCrea++;
						manaToUse +=  tour.getMe().getHandBOT2().elementAt(i).getCost();
					}
				}
				if (TOUR > 40 && amountOfCrea >= 3 && manaToUse >= 7 && tour.getOther().hasEmperorNightmareInBoard() == false){
					if (tour.getMe().hasEmperorNightmareInHandBOT2() == true){
						tour.getMe().removeEmperorFromHandBOT2();
					}
				}*/
					//	if (tour.getOther().getBoard().size() >1){////System.err.println(" test his life :" + tour.getOther().getBoard().elementAt(1).getDefense());}
						...
						if (tour.getMe().getBoard().size()>-1){
							...
							//answer = tour.summonPossibilities(answer,summonWithoutCharge,opponent,mine,PlayerBOT2Number,TOUR);
